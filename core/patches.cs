...
public static class PatchManager {
    ...
    private static (string filename, string gameTitle) GetPatchFileInfo(string region, PatchTarget target) {
        return region switch {
            "NTSC" when target == PatchTarget.Singleplayer => (NtscSpFile, "Ratchet & Clank: Up Your Arsenal Single Player (NTSC-U)"),
            "NTSC" when target == PatchTarget.Multiplayer => (NtscMpFile, "Ratchet & Clank: Up Your Arsenal Multiplayer (NTSC-U)"),
            "PAL" when target == PatchTarget.Singleplayer => (PalSpFile, "Ratchet & Clank 3 Single Player (PAL)"),
            "PAL" when target == PatchTarget.Multiplayer => (PalMpFile, "Ratchet & Clank 3 Multiplayer (PAL)"),
